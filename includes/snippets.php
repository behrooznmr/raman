...
function ra_add_megamenu_checkbox( $item_id, $item, $depth, $args ) {
	...
	?><p class="field-ra-is-megamenu description description-wide" style="margin:10px 0;"><label><input type="checkbox" name="ra_is_megamenu[<?php echo esc_attr($item_id); ?>]" value="1" <?php checked($is_mega, '1'); ?> /> <?php _e('فعال کردن مگامنو', 'ra'); ?></label></p><?php
}
...
class RA_Column_Walker extends Walker_Nav_Menu {
	function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
		$icon_html = '';
		foreach ($item->classes as $class) { if (strpos($class, 'icon-') === 0) { $icon_html = '<i class="ra-menu-icon ' . esc_attr($class) . '"></i> '; break; }}
		if ($item->url === '#') {
...
class RA_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	private function build_megamenu_content() {
        ...
        <div class="ra-mega-col"><?php wp_nav_menu(['theme_location' => 'megamenu_col_1', 'container' => false, 'walker' => new RA_Column_Walker()]); ?></div>
        <div class="ra-mega-col"><?php wp_nav_menu(['theme_location' => 'megamenu_col_2', 'container' => false, 'walker' => new RA_Column_Walker()]); ?></div>
        <div class="ra-mega-col"><?php wp_nav_menu(['theme_location' => 'megamenu_col_3', 'container' => false, 'walker' => new RA_Column_Walker()]); ?></div>
        </div><?php
		return ob_get_clean();
	}
}
...
function ra_pass_megamenu_data_to_js() {
	global $ra_megamenus_data;
	if (empty($ra_megamenus_data)) return;
